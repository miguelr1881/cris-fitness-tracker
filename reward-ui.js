...
export function createRewards(api) {
  ...
  function show() {
    ...
    ${progress.next ? `<section class="next-reward"><p class="eyebrow">SIGUIENTE RECOMPENSA</p><span class="reward-emblem">${icon(progress.next.icon)}</span><h3>${escape(progress.next.name)}</h3><p class="reward-description">${escape(progress.next.description || 'Un regalo de Miguel para Cristina.')}</p><p>${escape(rewardGoal(progress.next))}</p><div class="progress-track"><span style="width:${Math.min(100, progress.count / progress.next.days * 100)}%"></span></div><span class="reward-count">${progress.count} / ${progress.next.days} ${escape(METRICS[progress.next.metric || 'days'])}</span></section>` : '<p class="muted-copy">Todas las recompensas están desbloqueadas.</p>'}
  ...
  async function share(award, save = false) {
    ...
    context.font = '19px Manrope'; context.fillStyle = '#9987a5'; context.fillText('RECOMPENSA DESBLOQUEADA', 540, 358);
    const middle = 615;
  ...
  async function action(name, button) {
    ...
    if (name === 'preview-celebration') { celebrate({ ...DEFAULT_REWARDS[3], id:'preview', earnedAt:dateKey(), seen:true, redeemed:false, preview:true }); return true; }
    ...
    if (name === 'redeem-reward') {
      const next = structuredClone(getData());
      const award = next.rewardAwards.find(item => item.id === button.dataset.id); award.redeemed = !award.redeemed;
